...
def assign_attribute(stats):
    ...
    if stats.weapon in Knife_list:
        stats.slash = 1
        stats.pierce = 1
        stats.luc += int(stats.level / 5) * random.randint(1, 5)
        stats.dex += int(stats.level / 5) * random.randint(1, 5)

    if stats.weapon in Sword_list:
        stats.slash = 2
        stats.bash = 1
        stats.str += int(stats.level / 5) * random.randint(1, 5)
        stats.dex += int(stats.level / 5) * random.randint(1, 5)
    ...
    if stats.weapon in Wood_Club_list:
        stats.bash = 2
        stats.str += int(stats.level / 5) * random.randint(1, 5)
        stats.con += int(stats.level / 5) * random.randint(1, 5)

    if stats.weapon in Short_Bow_list:
        stats.pierce = 2
        stats.dex += int(stats.level / 4) * random.randint(1, 5)

    if stats.weapon in Catapult_list:
        stats.bash = 1
        stats.str += int(stats.level / 6) * random.randint(1, 5)
        stats.con += int(stats.level / 6) * random.randint(1, 5)

    if stats.weapon in Enhanced_Tree_Root_list:
        stats.pierce = 1
        stats.luc += int(stats.level / 6) * random.randint(1, 5)
        stats.dex += int(stats.level / 6) * random.randint(1, 5)

    if stats.weapon in Khopesh_list:
        stats.slash = 1
        stats.bash = 1
        stats.pierce = 1
        stats.str += int(stats.level / 8) * random.randint(1, 5)
    ...
    if stats.armor in Cloth_Armor_List:
        stats.defense += 1
